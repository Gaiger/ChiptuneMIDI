...
{
	do {
		if(CHANNEL_CONTROLLER_INSTRUMENT_NOT_SPECIFIED != get_channel_controller_pointer_from_index(voice)->instrument_code){
			if(number != get_channel_controller_pointer_from_index(voice)->instrument_code){
				CHIPTUNE_PRINTF(cMidiProgramChange, "tick = %u, voice = %d, MIDI_MESSAGE_PROGRAM_CHANGE: from %s(%d) to %s(%d)\r\n",
								tick, voice,
								get_instrument_name_string(get_channel_controller_pointer_from_index(voice)->instrument_code),
								get_channel_controller_pointer_from_index(voice)->instrument_code,
	...
	get_channel_controller_pointer_from_index(voice)->instrument_code = number;
	return 0;
}
...
int8_t s_ending_instrument_code_array[MIDI_MAX_CHANNEL_NUMBER];
...
static int process_timely_midi_events(void)
{
	...
	while(1)
	{
		uint32_t const event_tick = get_next_event_triggering_tick();
...
#define PHASER_LOW_FREQUENCY_OSCILLATION_LEVEL_DESCALE_BIT_NUMBER \
	(PHASER_LOW_FREQUENCY_OSCILLATION_LEVEL_FULL_SCALE_BIT_NUMBER - PHASER_ALLPASS_COEFFICIENT_RANGE_LEVEL_BIT_NUMBER)

#define DESCALE_PHASER_LOW_FREQUENCY_OSCILLATION_LEVEL(LEVEL) \
	((LEVEL) >> PHASER_LOW_FREQUENCY_OSCILLATION_LEVEL_DESCALE_BIT_NUMBER)
#define DESCALE_REMAINING_PHASER_LOW_FREQUENCY_OSCILLATION_SCALE(VALUE) \
	((VALUE) >> PHASER_ALLPASS_COEFFICIENT_RANGE_LEVEL_BIT_NUMBER)

#define SCALE_TO_PHASER_ALLPASS_COEFFICIENT_RANGE(LEVEL, RANGE) \
	DESCALE_REMAINING_PHASER_LOW_FREQUENCY_OSCILLATION_SCALE( \
		DESCALE_PHASER_LOW_FREQUENCY_OSCILLATION_LEVEL(LEVEL) * (RANGE))

static inline uint8_t calculate_phaser_allpass_coefficient(
...
#define PHASER_ALLPASS_STAGE_COEFFICIENT_DELTA		(12)
	...
	uint16_t coefficient = (uint16_t)(PHASER_ALLPASS_MIN_COEFFICIENT
									  + SCALE_TO_PHASER_ALLPASS_COEFFICIENT_RANGE(
											...
											PHASER_ALLPASS_COEFFICIENT_RANGE)
									  + stage_index * PHASER_ALLPASS_STAGE_COEFFICIENT_DELTA);
	return SATURATE_TO_INT8_MAX(coefficient);
}

/**********************************************************************************/
#define ALLPASS_FILTER_OUTPUT(INPUT, STATE, COEFFICIENT) \
	((STATE) - DIVIDE_BY_128((int64_t)(COEFFICIENT) * (INPUT)))

#define ALLPASS_FILTER_STATE(INPUT, OUTPUT, COEFFICIENT) \
	((INPUT) + DIVIDE_BY_128((int64_t)(OUTPUT) * (COEFFICIENT)))

static inline void apply_allpass_filter_to_wave_amplitude(int32_t * const p_wave_amplitude,
										int32_t * const p_filter_state,
										uint8_t const coefficient)
{
	int32_t const input = *p_wave_amplitude;
	int32_t const output = ALLPASS_FILTER_OUTPUT(input, *p_filter_state, coefficient);

	*p_filter_state = ALLPASS_FILTER_STATE(input, output, coefficient);
	*p_wave_amplitude = output;
}
...
void process_phaser_filter(oscillator_t * const p_oscillator)
{
	do {
		...
		for(int i = 0; i < PHASER_ALLPASS_STAGE_NUMBER; i++){
			...
			apply_allpass_filter_to_wave_amplitude(&p_oscillator->mono_wave_amplitude,
...
static int32_t generate_32bit_wave(void)
{
	...
	do
	{
		...
		if(-1 == process_timely_midi_message_and_events()){
	...
	{
		if(true == is_stereo()
				&& true == is_processing_left_channel()){
			break;
		}
								...
								bool is_channel_has_note_array[MIDI_MAX_CHANNEL_NUMBER])
{
	...
	do
	{
		if(TO_LAST_MESSAGE_INDEX == end_midi_message_index){
		...
		if(end_midi_message_index != s_midi_messge_index){
			CHIPTUNE_PRINTF(cDeveloping, "WARNING :: chase_midi_messages sought = %ddoes not meet targeted = %d\r\n",
							s_midi_messge_index, end_midi_message_index);
			ret = -1;
		}
	}while(0);
...
void chiptune_prepare_session(uint32_t const resolution)
{
	...
	do
	{
		...
		for(int8_t voice = 0; voice < MIDI_MAX_CHANNEL_NUMBER; voice++){
			s_ending_instrument_code_array[voice] = get_channel_controller_pointer_from_index(voice)->instrument_code;
			if(CHIPTUNE_INSTRUMENT_NOT_SPECIFIED == s_ending_instrument_code_array[voice]){
				if(false == is_channel_has_note_array[voice]){
					s_ending_instrument_code_array[voice] = CHIPTUNE_INSTRUMENT_UNUSED_CHANNEL;
				}
			}
		}
	}while(0);
...
int8_t chiptune_get_channel_instrument(int8_t const channel_index)
{
	...
	return get_channel_controller_pointer_from_index(channel_index)->instrument_code;
}
...
int chiptune_get_ending_instruments(int8_t instrument_code_array[MIDI_MAX_CHANNEL_NUMBER])
{
	...
	memcpy(&instrument_code_array[0], &s_ending_instrument_code_array[0], MIDI_MAX_CHANNEL_NUMBER * sizeof(int8_t));
	return 0;
}
